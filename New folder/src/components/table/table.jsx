...
function Table() {
	...
	function checkBoxClick(event) {
		...
		let CheckedUser;
		console.log(user, "without check");
		if (name == "selectall") {
	...
	function handleDeleteIconClick(id) {
		let newItems = allUsers.filter(function (item) {
			return item.id != id;
		});
		dispatch(allActions.setUser(newItems, [...userForFilter]));
	}

	function handleChange(e, data) {
		let modifiedfield = e.target.name;

		let changedItem = user.find((item) => {
			return item.id == data.id;
		});
		console.log(changedItem, "ggg");
		if (modifiedfield == "name") changedItem.name = e.target.value;

		if (modifiedfield == "email") changedItem.email = e.target.value;

		if (modifiedfield == "role") changedItem.role = e.target.value;
